...
import { CustomerInterface, CustomerFormValidation, CustomerFormValidationInterface } from '../../types/Customer/CustomerInterface';
...
onMounted(() => {
  axios.get(import.meta.env.VITE_BACKEND_API_ENDPOINT + '/customer/' + props.uuid)
...
function submitCustomer() {
  axios.put(
    import.meta.env.VITE_BACKEND_API_ENDPOINT + '/customer/' + props.uuid,
     {
    ...
    .then((response: AxiosResponse) => {
      if(response.status === 202) {
        ElNotification({
          ...
          message: "<p class='text-left'>" + t('pages.customer.edit.notifications.success.message') + "</p>",
          type: 'success',
        });
...
const isMyCustomer = computed<Boolean | void>(() => {
  if(auth.ready()) {
    return props.uuid === auth.user().customer.uuid ? true : false;
  }
})
...
<template>
  ...
  <div v-if="undefined !== customerProfile" class="col-span-full xl:col-span-8 bg-white shadow-lg rounded-sm border border-gray-200 sm:overflow-hidden sm:rounded-md">
    <header class="px-5 py-4 border-b border-gray-100">
      <h2 class="font-semibold text-gray-800">{{ isMyCustomer ? t("pages.customer.edit.my-profile") : t("pages.customer.edit.edit-of") + customerProfile.corporateName }}</h2>
    </header>


    <Form class="space-y-6 px-4 py-5 sm:p-6" @submit="submitCustomer">
      <div v-for="field in ['corporateName', 'tradeName', 'streetAddress', 'postalCode', 'locality', 'country']" class="grid grid-cols-3 gap-6">
        <div class="col-span-3 sm:col-span-2">
          <label :for="'customer-' + field" class="block text-sm font-medium text-gray-700">{{ t('customer.label.' + field) }}</label>
