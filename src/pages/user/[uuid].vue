...
onMounted(() => {
  axios
    .get(`${import.meta.env.VITE_BACKEND_API_ENDPOINT}/user/${props.uuid}`)
...
function submitUser() {
  axios
    .put(`${import.meta.env.VITE_BACKEND_API_ENDPOINT}/user/${props.uuid}`, {
...
const isMyProfile = computed<Boolean>(() => {
  if (auth.ready()) {
    return props.uuid === auth.user().uuid;
  }
  return false;
});
</script>
  ...
  >
    <header class="px-5 py-4 border-b border-gray-100">
      <h2 class="font-semibold text-gray-800">
        {{
          isMyProfile
            ? t('pages.user.edit.my-profile')
            : t('pages.user.edit.edit-of') + userProfile.firstName + ' ' + userProfile.lastName
        }}
      </h2>
    </header>
    ...
    <Form class="space-y-6 px-4 py-5 sm:p-6" @submit="submitUser">
      ...
      <div
        v-for="field in ['firstName', 'lastName', 'phoneNumber', 'cellPhoneNumber']"
        :key="field"
        class="grid grid-cols-3 gap-6"
      >
        <div class="col-span-3 sm:col-span-2">
          <label :for="'user-' + field" class="block text-sm font-medium text-gray-700"
            >{{ t('user.label.' + field) }}
            <div class="mt-1 flex rounded-md shadow-sm">
              <Field
                :id="'user-' + field"
